...
const BASE = 'https://straydog.surge.sh'

test.describe('進行順 画面フロー (Surge)', () => {
  ...
  test('10. コンプリート画面', async ({ page }) => {
    await page.goto(BASE + '/#debug/screen/complete')
  ...
  test('11. ストーリーモーダル', async ({ page }) => {
    await page.goto(BASE + '/#debug')
    await page.waitForTimeout(1000)
    await page.evaluate(() => localStorage.setItem('sd_intro_done', 'true'))
    await page.waitForTimeout(500)
  ...
  test('12. ストーリーマラソン', async ({ page }) => {
    await page.goto(BASE + '/#debug/story/marathon')
